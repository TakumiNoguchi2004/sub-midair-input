...
> ⚠️ **このブランチ `demo/fold-input` はデモ版です。**
...
> 日本語/英語の「試作」は `demo/fold-input` ブランチの Web UI 内実装を指す（backend パッケージは未作成）。詳細は [`docs/fold_input_demo.md`](docs/fold_input_demo.md) / [`docs/japanese_input.md`](docs/japanese_input.md)。
...
- [`docs/fold_input_demo.md`](docs/fold_input_demo.md) — **`demo/fold-input` ブランチのデモ内容**（折り曲げ式 日本語/英語入力・言語切替・入力テスト・UI 日英切替・調整項目）
