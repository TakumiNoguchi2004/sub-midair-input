...
> **注記 (2026-06 更新)**: 本ファイルは *実装前の構想メモ* であり、現行の実体とは異なる箇所がある。
> 実装は uv workspace monorepo (`packages/*`) として構成され、統合 CLI は `uv run midair --mode emoji ...`、
> データは `data/emoji_search/` 配下に置く。**最新の構成は [README.md](README.md) / [CLAUDE.md](CLAUDE.md) を参照**。
> 以降の 7・8 章 (CLI 仕様・ディレクトリ構成) は初期構想のまま残している。
