...
- **方針**: 各モダリティを独立サブシステムとして**分割開発**し、最終的に 1 つの統合アプリへ
  **疎結合のまま束ねる**。
- **共通構造 (検索が要るモダリティ)**: `入力 → 埋め込みベクトル化 → FAISS 近傍検索 → 候補表示`。
  ただしこれは**「軌跡/テキストを検索して当てる」モダリティ (絵文字) の話**。入力の読み取り自体はフロントで完結する (下記)。
...
> **入力の読み取り (手の検出 → ジェスチャ/フリックの解釈 → 文字の確定) 自体はフロント (HTML/JS) で完結できる。**
> ジェスチャ/姿勢を決定論的に文字へ写像する入力 — 日本語の50音フリック、言語切替モーション、
> 将来の英語のフリック/エアタイプ等 — は **backend 不要**。
> **backend (検索層) が要るのは「軌跡やテキストを認識・検索して候補を当てる」ときだけ**で、
> 絵文字がまさにそれ (手書き/テキスト → CLIP 埋め込み + FAISS で絵文字候補を当てる)。

### 入力モードとジェスチャ解釈

カメラ入力の同じ手の動きは、現在の入力モードによって別の意味として扱う。
絵文字モードでは人差し指のみで描画 / グーパーグーで検索 / グーフリップでクリア、日本語モードでは
...
[ブラウザ]                                   [サーバ: midair-web]            [検索層]
 手 → MediaPipe → ジェスチャ/座標
   人差し指のみ=描画 / グーパーグー=検索 / グーフリップ=クリア
