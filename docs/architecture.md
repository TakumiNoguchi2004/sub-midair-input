...
```
            midair-shared  ← 全パッケージが依存する共通基盤 (契約 + FAISS utils)
              ▲   ▲   ▲
   ┌──────────┘   │   └───────────┐
emoji-search  japanese-search  english-search   ← モダリティ別サブシステム (Searcher を実装)
   ▲   ▲             ▲                ▲
   │   └─────────────┼────────────────┘
...
### `packages/japanese-search` → `japanese_search` (日本語入力 / スケルトン)
**スコープ**: 日本語モダリティ。現状は `JapaneseSearcher`(`Searcher` 契約) の骨だけで
`NotImplementedError`。実装時は emoji と同じ構造 (encoder / data / searcher + 必要なら scripts) を踏襲する。

### `packages/english-search` → `english_search` (英語入力 / スケルトン)
**スコープ**: 英語モダリティ。japanese と同様のスケルトン。
